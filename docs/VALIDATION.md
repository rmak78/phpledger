...
| Publisher signatures | `tests/release-signing-test.php`: **20 checks passed**, including signature/member tampering, unsafe/private entries, channel identity, output overwrite and fixed official-download host restrictions. Test keys are synthetic. |
...
| Installer browser | Actual synthetic fresh-install journey through host/database review, all 32 existing migrations, private configuration/OAuth, first account and onboarding; desktop 1440px, tablet 768px and phone 390px checks. Completed setup is locked. Screenshots are in `output/playwright/stable-path/installer-*.png`. This was developer-operated, not unfamiliar-user acceptance. |
...
Validation used synthetic data and separate disposable Compose projects. The final complete suite passed 225 tests on each of PHP 8.2.33 and 8.3.33, with clean PHP lint/static analysis, fresh and populated-upgrade rehearsals, and a tested backup/restore. The browser journey connected invoice/credit/payment and order/receipt/bill/return/stock-sale flows to the existing bank reconciliation; five bank rows reconciled to USD 884.25 with no difference. Exact final counts, canonical migration-byte checks and responsive evidence are in the linked receipt.

Migrations and schema changes: yes, isolated local checks and the authorized hosted-demo upgrade. The package and hosted application are published; the receipt records website publication separately. Live verification used one synthetic demo visitor; no real customer payments or messages were sent. Technical checks do not represent professional accounting or tax approval. Local references include the approved product/module direction, multi-currency, party/vetting and correction model documents; no Google Drive document was required or read.
