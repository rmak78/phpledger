...
2. **phpledger.com:** the rebuilt website (`www/website/public`, 68 pages, download page and news article for 1.0.0, 1.0.0 captures) was not deployed. The hosted demo was cut over on 19 September 2026 (see below). The previous releases' deployment tooling in `.cache/` targets the production host with credentials from Windows Credential Manager; run it or deploy manually, then record the website release id and demo cutover here.
