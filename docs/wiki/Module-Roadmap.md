...
**0.1.3-preview publication:** the owner authorized the completed foundation/module/running-balance changes to go live first. Current release status is recorded in the repository's `docs/repository/sprint-05/PREVIEW-0.1.3-VALIDATION.md`; earlier local wording below is historical. The next implementation order remains API/MCP reads, controlled commands, then optional AR/AP. Subledgers and reviewed statement packages are unfinished. SEO discovery and campaign preparation are high priority in parallel.

PHP Ledger's required core lets an authorized user manage accounts, record and reverse journals, reconcile balances and complete the supported accounting period with every optional module disabled. The 0.1.3-preview release consolidates opening/cutover, reasoned periods, bank reconciliation, core CSV exports and the bundled module lifecycle. Its account ledger shows running balances on desktop, tablet and mobile.
