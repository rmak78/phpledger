...
The support enquiry prepares a reviewable email draft or copy on the visitor's device. It has no submission endpoint, storage, automatic messaging or payment. All product images use sample records; photograph subjects are illustrative. New source, asset and responsive/keyboard checks are recorded in [website QA](../../../www/website/design-qa.md). The earlier composition contract below is preserved as historical context.
...
Three deliberate controls switch between **Record expense**, **Review entry**, and **See report**. Use actual captures of the same verified sample transaction once the working sprint is available, with a visible sample-data label and readable enlarged view. The captured amount, currency, source reference and report date must agree. No autoplay, fake animation of financial posting, or simulated public demo sign-in is required.

Before real captures are supplied, any displayed approved concept must explicitly say **Product design preview · Sample data**. Once replaced, use **Development preview · Sample company data**. Neither label means a stable release or a hosted accounting service is available.
...
Actions: **Ask about support** leads to the approved email contact; **View the repository** leads to `https://github.com/phpledger/phpledger`. Keep release/setup statements synchronized with verified repository guidance, and do not link to unpushed files as though they exist publicly.
