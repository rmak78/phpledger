...
Owner decisions adopted on 15 September 2026 from the supplied local `docs/strategy/DECISION-REGISTER.md`, section D. The accounting core stays open source; optional modules and commercial licensing follow [Licensing policy](LICENSING-POLICY.md). These decisions supersede the earlier order that put controlled commands and the installer before AR/AP. These are delivery priorities and gates, not new implemented features or dates.
...
| 5. Pakistan tax adapter and FBR client | Reviewed entity/effective-period rules, immutable tax snapshots and an open-source FBR digital-invoicing client: sandbox scenario runner SN001–SN028, protected token storage, HS/UoM reference sync, QR/logo invoice printing, the applicable 72-hour edit-window workflow, debit/credit notes, and Annex-A/C/I built from ledger plus DI responses. | Current PRAL/FBR contract and sandbox verification, reconciled document/tax/DI outcomes, failure/retry/correction tests, source provenance and qualified local review for the supported scope. Actual submissions require separate configured authority. |
...
Project-owned current core/POS code uses AGPL-3.0-or-later; published 0.1.x previews retain MIT. [Licensing policy](LICENSING-POLICY.md) governs commercial licensing and future declared commercial modules, superseding the former no-paid-feature-tier statement. Dependency and legacy notices remain separate.
