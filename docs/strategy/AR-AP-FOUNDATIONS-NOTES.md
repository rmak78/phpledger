...
The owner approved the foundation plan after parallel read-only review. The starting commit is `531541c` on `master`; the former `codex/integration-delivery` branch was deliberately removed during cleanup. Implementation uses `codex/ar-ap-foundations`. Work is local; no push, deployment, messages, providers or payments are authorized by this task.
