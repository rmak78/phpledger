...
test('starter has required AR AP and independent optional purchasing and inventory capabilities',function():void {
    ...
    $f=ledger_fixture();
    assert_throws(fn()=>pl_set_company_module($f['actor_id'],$f['company_id'],'ar',false,0,$modules['ar']['digest'],'Synthetic','ar-disabled'),DomainException::class,'required');
    assert_throws(fn()=>pl_set_company_module($f['actor_id'],$f['company_id'],'purchasing',true,0,$modules['purchasing']['digest'],'Synthetic','buy-enabled'),DomainException::class,'dependency');
});
...
test('hiding AR AP is audited owner presentation only and preserves required services',function():void {
    ...
    $result=pl_set_company_visibility($f['actor_id'],$f['company_id'],false,false,0,'Synthetic simpler navigation','visibility');
    assert_true($result==pl_set_company_visibility($f['actor_id'],$f['company_id'],false,false,0,'Synthetic simpler navigation','visibility'));
