"""Exercise module administration and gated POS through local HTTP with synthetic records.

Only http://127.0.0.1:18200 and its existing local Compose database are used.
Creates independent owner/viewer identities in memory, retains synthetic books,
...
def run() -> dict:
    ...
    accounting.php_local(
        f"$p=pl_preview_opening({actor},{company},{book}, ['cutover_date'=>'2026-01-01','source'=>'Synthetic module acceptance',"
    ...
    enable = form.fields | {"enabled": "1", "reason": "Synthetic owner enables POS"}
    ...
    disable = owner.request("/modules").markup.form_for("/modules").fields | {"enabled": "0", "reason": "Synthetic owner disables POS"}
    disabled = owner.request("/modules", disable)
    check("Owner disables module and retains history", disabled.status == 200 and "Disabled" in disabled.body and "Synthetic owner enables POS" in disabled.body)
    ...
    reenable = owner.request("/modules").markup.form_for("/modules").fields | {"enabled": "1", "reason": "Synthetic owner reenables POS"}
