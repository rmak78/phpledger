...
function inventory_fixture(): array
{
    ...
    pl_set_company_module($f['actor_id'], $f['company_id'], 'inventory', true, 0, $manifest['digest'], 'Synthetic stock module', bin2hex(random_bytes(16)));
    foreach (['inventory' => ['1300','asset'], 'grni' => ['2100','liability'], 'variance' => ['5200','expense']] as $name => [$code,$type]) {
        $account = pl_save_account($f['actor_id'], $f['company_id'], $f['book_id'], ['code' => $code, 'name' => 'Synthetic ' . $name, 'type' => $type, 'role' => null,
            'is_active' => true, 'reason' => 'Synthetic inventory chart', 'creation_key' => bin2hex(random_bytes(16))]);
...
{
    return ['sku' => 'SYNTH-' . bin2hex(random_bytes(4)), 'name' => 'Synthetic stock item', 'kind' => 'stock', 'base_unit' => 'each', 'selling_price' => '25', 'is_active' => true,
        ...
        'reason' => 'Synthetic product fixture', 'idempotency_key' => bin2hex(random_bytes(16))];
}
...
function inventory_move_input(array $f, string $quantity = '10', string $value = '100', string $date = '2026-01-05'): array
{
    return ['product_id' => $f['product_id'], 'quantity' => $quantity, 'amount_base' => $value, 'date' => $date, 'offset_account_id' => $f['grni_account_id'],
        'source_type' => 'test_stock', 'source_reference' => bin2hex(random_bytes(16)), 'reason' => 'Synthetic inventory movement', 'idempotency_key' => bin2hex(random_bytes(16))];
}
...
test('stock rejects negative balances backdating closed periods duplicate sources and outer failures atomically', function (): void {
    ...
    assert_throws(function () use ($f): void { pl_ledger_transaction(function () use ($f): void { pl_inventory_receive($f['actor_id'], $f['company_id'], $f['book_id'], inventory_move_input($f)); throw new DomainException('Synthetic outer rollback'); }); });
...
test('inventory enabled writes and historical reads enforce company permissions', function (): void {
    ...
    pl_set_company_module($f['actor_id'], $f['company_id'], 'inventory', false, 1, pl_module_registry()['inventory']['digest'], 'Synthetic disable', bin2hex(random_bytes(16)));
...
test('reviewed stock opening conversion links existing value without reposting the GL', function (): void {
    ...
    $opening = pl_preview_opening($f['actor_id'], $f['company_id'], $f['book_id'], ['cutover_date' => '2026-01-01', 'source' => 'Synthetic stock cutover', 'unpaid_documents' => [],
    ...
    $input = ['date' => '2026-01-01', 'reason' => 'Synthetic reviewed stock count and invoice evidence', 'lines' => [['product_id' => $f['product_id'], 'quantity' => '10', 'amount_base' => '100']]];
    ...
    assert_throws(fn() => pl_reverse_opening($f['actor_id'], $f['company_id'], $f['book_id'], (int) $cutover['id'], 'Synthetic forbidden opening restart'), DomainException::class, 'inventory');
});
...
test('invoice stock adapter is atomic and its reversal restores original issue cost', function (): void {
    ...
    $issued = pl_inventory_issue_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $document, $journal['id'], 'synthetic-invoice-123');
    ...
    pl_inventory_reverse_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $document, '2026-09-15', 'synthetic-reverse-123', 'Synthetic invoice correction');
...
test('reversing a customer credit restores the invoice returnable basis without rewriting movements', function (): void {
    ...
    $journal = pl_post_journal($f['actor_id'], $f['company_id'], $f['book_id'], ledger_payload($f, '75'));
    $document = ['id' => 123, 'document_date' => '2026-09-14', 'journal_id' => $journal['id'], 'lines' => [['product_id' => $f['product_id'], 'quantity' => '3']]];
    pl_inventory_issue_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $document, $journal['id'], 'synthetic-invoice-123');
    ...
    pl_inventory_credit_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $credit, $document, 'synthetic-credit-124');
    assert_same('8.0000', pl_inventory_balance($f['actor_id'], $f['company_id'], $f['book_id'], $f['product_id'])['quantity']);
    pl_inventory_reverse_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $credit, '2026-09-15', 'synthetic-reverse-credit-124', 'Synthetic credit correction');
    assert_same('7.0000', pl_inventory_balance($f['actor_id'], $f['company_id'], $f['book_id'], $f['product_id'])['quantity']);
    pl_inventory_reverse_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $document, '2026-09-15', 'synthetic-reverse-invoice-123', 'Synthetic invoice correction');
    assert_same('10.0000', pl_inventory_balance($f['actor_id'], $f['company_id'], $f['book_id'], $f['product_id'])['quantity']);
});

test('real stock invoices and credits post and reverse financial and physical effects together', function (): void {
    $f = inventory_fixture();
    $party = pl_save_party($f['actor_id'], $f['company_id'], $f['book_id'], ['legal_name' => 'Synthetic stock customer', 'entity_type' => 'private_company', 'country_code' => 'GB', 'is_customer' => true, 'is_vendor' => false, 'currency' => 'USD', 'request_key' => bin2hex(random_bytes(16)), 'reason' => 'Synthetic stock invoice test']);
    ...
    $input = ['kind' => 'invoice', 'party_id' => $party['id'], 'date' => '2026-01-06', 'due_date' => '2026-02-06', 'currency' => 'USD', 'creation_key' => bin2hex(random_bytes(16)),
        'lines' => [['description' => 'Synthetic stock sale', 'product_id' => $f['product_id'], 'quantity' => '3', 'unit_price' => '25', 'account_id' => $f['accounts']['4000']]]];
    ...
    pl_reverse_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $credit['id'], null, 'Synthetic credit cancellation');
    assert_same('7.0000', pl_inventory_balance($f['actor_id'], $f['company_id'], $f['book_id'], $f['product_id'])['quantity']);
    pl_reverse_ar_document($f['actor_id'], $f['company_id'], $f['book_id'], $invoice['id'], null, 'Synthetic invoice cancellation');
    assert_same('10.0000', pl_inventory_balance($f['actor_id'], $f['company_id'], $f['book_id'], $f['product_id'])['quantity']);
