...
{
    $f=ledger_fixture();
    $f['event_id']=pl_ledger_transaction(fn()=>pl_enqueue_outbound_event($f['company_id'],$f['book_id'],'party.created','party',1,1,'sample-event'));
    return $f;
}

function outbound_handler(array $f,callable $handler): array
{
    return ['sample'=>['company_id'=>$f['company_id'],'book_id'=>$f['book_id'],'handler'=>$handler]];
}
...
test('outbound content is transactional immutable and content-bound',function():void{
    ...
    assert_same($f['event_id'],pl_ledger_transaction(fn()=>pl_enqueue_outbound_event($f['company_id'],$f['book_id'],'party.created','party',1,1,'sample-event')));
    assert_throws(fn()=>pl_ledger_transaction(fn()=>pl_enqueue_outbound_event($f['company_id'],$f['book_id'],'party.created','party',1,2,'sample-event')),DomainException::class);
...
test('scoped fake outbound dispatch acknowledges once outside transaction',function():void{
    ...
    assert_same(0,pl_dispatch_outbound_events($registry)['claimed']);assert_same(['sample-event'],$seen);
...
test('outbound failure retries with capped backoff and eight-attempt terminal state',function():void{
    $f=outbound_fixture();$registry=outbound_handler($f,static function():bool{throw new RuntimeException('Sample private response must not be logged');});
...
test('outbound leases recover and old worker cannot acknowledge replacement',function():void{
    $f=outbound_fixture();
    DB::insert('pl_outbound_deliveries',['event_id'=>$f['event_id'],'company_id'=>$f['company_id'],'book_id'=>$f['book_id'],'consumer'=>'sample']);
    $first=pl_outbound_claim('sample',$f['company_id'],$f['book_id']);
    assert_same(null,pl_outbound_claim('sample',$f['company_id'],$f['book_id']));
    DB::query('UPDATE pl_outbound_deliveries SET leased_until=DATE_SUB(UTC_TIMESTAMP(),INTERVAL 1 SECOND) WHERE id=%i',$first['id']);
    $replacement=pl_outbound_claim('sample',$f['company_id'],$f['book_id']);
...
test('separate outbound workers claim disjoint events and cannot claim a live lease twice',function():void{
    $f=outbound_fixture();
    $otherId=pl_ledger_transaction(fn()=>pl_enqueue_outbound_event($f['company_id'],$f['book_id'],'party.updated','party',1,2,'sample-second-event'));
    foreach([$f['event_id'],$otherId] as $eventId){DB::insert('pl_outbound_deliveries',['event_id'=>$eventId,'company_id'=>$f['company_id'],'book_id'=>$f['book_id'],'consumer'=>'sample']);}
...
test('fake receiver deduplicates delivered event after lost acknowledgement and expired lease',function():void{
    ...
    $original=pl_outbound_claim('sample',$f['company_id'],$f['book_id']);
