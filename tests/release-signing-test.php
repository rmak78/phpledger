...
/** Entirely sample publisher keys and ZIPs, never a trusted production key. */
$directory = sys_get_temp_dir() . '/phpledger-signing-' . bin2hex(random_bytes(8));
if (!mkdir($directory, 0700)) { throw new RuntimeException('Cannot create sample workspace.'); }
$checks = 0;
...
try {
    ...
    signing_assert(pl_update_download_url_allowed('https://release-assets.githubusercontent.com/example.zip?token=sample'));
    ...
    if ($key === false || !openssl_pkey_export($key, $private)) { throw new RuntimeException('Cannot create sample key.'); }
    file_put_contents($directory . '/sample.pem', $private);
    $details = openssl_pkey_get_details($key);
    if ($details === false) { throw new RuntimeException('Cannot export sample public key.'); }
    $content = '<?php // original sample payload';
    ...
    pl_release_sign_update($directory . '/release.zip', $directory . '/sample.pem', $directory . '/release.update.json');
    ...
    signing_reject(fn () => pl_release_sign_update($directory . '/release.zip', $directory . '/sample.pem', $directory . '/release.update.json'));
    ...
    signing_zip($directory . '/private.zip', $files + ['www/phpledger/includes/config.local.php' => 'sample configuration']);
