...
// Synthetic data only, on the disposable Compose database. No historical SQL imports.
...
$name='Synthetic UI list capacity 0.6';
...
if ($measureOnly && !$company) { throw new RuntimeException('Seed the synthetic fixture before measuring.'); }
if ($company) {
    ...
    $actor=pl_create_user('list-capacity-'.$suffix.'@example.test','Synthetic list capacity tester','Synthetic-only-'.$suffix);
    ...
    pl_ledger_transaction(function () use ($actor,$companyId,$bookId,$accounts,$start): void {
        for ($i=$start;$i<$start+100;$i++) {
            pl_save_document($actor,$companyId,$bookId,['kind'=>'receipt','date'=>'2026-09-14','amount'=>'1','money_account_id'=>$accounts['1000'],'category_account_id'=>$accounts['4000'],
                'counterparty'=>'Synthetic capacity customer','reference'=>'CAPACITY-'.$i,'memo'=>'Synthetic paged-list evidence','creation_key'=>'list-capacity-06:receipt:'.$i]);
            $draft=pl_save_general_draft($actor,$companyId,$bookId,['date'=>'2026-09-14','description'=>'Synthetic capacity journal','reference'=>'CAPACITY-'.$i,'creation_key'=>'list-capacity-06:journal:'.$i,
                'lines'=>[['account_id'=>$accounts['1000'],'debit'=>'1','credit'=>'0','description'=>'Synthetic cash'],['account_id'=>$accounts['3000'],'debit'=>'0','credit'=>'1','description'=>'Synthetic equity']]]);
...
for ($batch=0;!$measureOnly && $batch<10;$batch++) {
    ...
    $account=pl_save_account($actor,$companyId,$bookId,['code'=>$code,'name'=>'Synthetic capacity bank '.$batch,'type'=>'asset','role'=>'cash_bank','currency'=>'USD','is_active'=>true,'reason'=>'Synthetic list capacity','creation_key'=>'list-capacity-06:bank:'.$batch]);
    $rows=[];
    for ($i=0;$i<500;$i++) { $rows[]=['date'=>'2026-09-14','reference'=>'CAPACITY-BANK-'.$batch.'-'.$i,'description'=>'Synthetic bank row','money_in'=>'1','money_out'=>'0']; }
...
if ((int)DB::queryFirstField('SELECT COUNT(*) FROM pl_bank_statement_rows WHERE company_id=%i AND book_id=%i',$companyId,$bookId)!==5000) { throw new RuntimeException('Expected 5000 synthetic bank rows.'); }
$evidence=[];
