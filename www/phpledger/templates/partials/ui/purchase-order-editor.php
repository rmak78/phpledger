...
<a class="btn btn-ghost" href="<?= pl_e(pl_url('/purchasing',$filters)) ?>">Cancel</a>
...
<?php }); ?>
<form id="purchase-order-editor" method="post" action="<?= pl_e(pl_url('/purchasing',$filters)) ?>" class="doc-body" data-commercial-form>
...
<?php if ($form['message']!==''): ?><div class="alert alert-danger" role="alert" tabindex="-1" data-form-error><?= pl_e($form['message']) ?><p>Your entered values are retained.<?php if ($order): ?> <a href="<?= pl_e(pl_url('/purchasing',['id'=>$order['id'],'return_filters'=>$filters])) ?>">Reload the current order</a> to resolve a revision conflict.<?php endif; ?></p></div><?php endif; ?>
...
<div class="flex justify-end mt-3" data-fold="document total" data-commercial-grand><?php pl_ui_totals(['Total order value ('.$company['currency'].')'=>$order?pl_money($order['total']):'â€”']); ?></div>
