...
<?php pl_ui_document_header($draft ? $draft['number'] . ' · Edit draft' : 'New general journal', 'draft', static function () use ($company, $draft): void { ?>
<a class="btn btn-ghost" href="<?= pl_e(pl_url($draft ? '/general-journals/detail' : '/general-journals', $draft ? ['id'=>$draft['id']] : [])) ?>">Cancel</a>
...
<?php if ($hasFailure): ?><div class="alert alert-danger" role="alert" tabindex="-1" data-form-error><strong>Your draft was not saved or posted.</strong><p><?= pl_e(pl_web_text($form, 'message')) ?></p><p>Your submitted values are retained.<?php if ($draft): ?> <a href="<?= pl_e(pl_url('/general-journals/edit', ['id' => $draft['id']])) ?>">Reload the latest saved version</a> before resolving a revision conflict.<?php endif; ?></p></div><?php endif; ?>
