...
?>
<section class="page-wrap setup-wizard" aria-labelledby="onboarding-title">
    <div class="page-heading">
        <div>
            <p class="eyebrow">Business setup · Step <?= $preview ? '5 of 6' : $activeStep . ' of 6' ?></p>
            <h1 id="onboarding-title"><?= $preview ? 'Preview your setup' : ($steps[$activeStep] ?? 'Set up a business') ?></h1>
            <p class="muted"><?= $preview ? 'Check the business identity, period and chart choice before anything is created.' : 'Set up the decisions that shape these books, one focused step at a time.' ?></p>
        </div>
        <a class="button secondary" href="<?= pl_e(pl_url('/companies')) ?>">Back to businesses</a>
    </div>

    <ol class="setup-progress" aria-label="Business setup progress">
        <?php foreach ($steps as $number => $label): ?>
            <?php $state = $number < ($activeStep + 1) ? ' is-complete' : ''; $current = $number === ($preview ? 5 : $activeStep) ? ' aria-current="step"' : ''; ?>
            <li class="setup-progress-item<?= $state ?>"<?= $current ?>><span><?= $number ?></span><b><?= pl_e($label) ?></b></li>
        <?php endforeach; ?>
    </ol>
    ...
    <?php if (!$preview && $activeStep <= 4): ?>
        <form action="<?= pl_e(pl_url('/onboarding')) ?>" method="post" class="panel form-grid setup-step-panel">
            <?= pl_csrf_field() ?>
            ...
            <?php elseif ($activeStep === 2): ?>
                <div class="field full-width"><label for="business-name">Business name</label><input id="business-name" name="name" type="text" maxlength="160" autocomplete="organization" required value="<?= pl_e($businessName) ?>" aria-describedby="business-name-help"><p class="muted" id="business-name-help">Use the name your team will recognise. A sample company receives a separate fictional name.</p></div>
                <div class="field"><label for="base-currency">Base currency</label><select id="base-currency" name="currency" required aria-describedby="currency-help"><?php foreach (pl_base_currency_options() as $code => $label): ?><option value="<?= pl_e($code) ?>"<?= $baseCurrency === $code ? ' selected' : '' ?>><?= pl_e($label) ?></option><?php endforeach; ?></select><p class="muted" id="currency-help">This book uses one functional currency. Currency selection does not activate country tax rules.</p></div>
                <div class="field"><label for="accounting-start">Accounting start date</label><input id="accounting-start" name="start_date" type="date" required value="<?= pl_e($startDate) ?>" aria-describedby="start-date-help"><p class="muted" id="start-date-help">The first date for these books. Accounting dates do not shift with time zones.</p></div>
            <?php elseif ($activeStep === 3): ?>
                <div class="field full-width"><label for="entity-type">Business or entity type</label><select id="entity-type" name="entity_type" required aria-describedby="entity-type-help"><?php foreach ($entityTypes as $key => $label): ?><option value="<?= pl_e($key) ?>"<?= $entityType === $key ? ' selected' : '' ?>><?= pl_e($label) ?></option><?php endforeach; ?></select><p class="muted" id="entity-type-help">This guides the period explanation only. It does not decide legal status or activate tax rules.</p></div>
                <div class="field full-width" data-fiscal-year-end>
                    <label for="fiscal-year-end-choice">Financial year end</label>
                    <select id="fiscal-year-end-choice" name="fiscal_year_end_choice" required aria-describedby="fiscal-help" data-fiscal-year-end-choice data-fiscal-custom-target="#fiscal-year-end-custom">
                    ...
                    <div class="fiscal-custom-group" data-fiscal-custom-group>
                        <label for="fiscal-year-end-custom">Custom year end</label>
                        <input id="fiscal-year-end-custom" name="fiscal_year_end_custom" type="text" inputmode="numeric" maxlength="5" value="<?= pl_e($fiscalYearEndCustom) ?>" placeholder="MM-DD" aria-describedby="fiscal-help">
                    </div>
            ...
            <div class="actions full-width">
                <?php if ($activeStep > 1): ?><a class="button secondary" href="<?= pl_e(pl_url('/onboarding?step=' . ($activeStep - 1))) ?>">Back</a><?php endif; ?>
                <button class="button primary" type="submit"><?= $activeStep === 4 ? 'Preview chart and setup' : 'Continue' ?></button>
                <?php if ($activeStep === 1): ?><a class="button secondary" href="<?= pl_e(pl_url('/companies')) ?>">Cancel</a><?php endif; ?>
            </div>
        </form>
    <?php elseif ($preview): ?>
        <div class="panel setup-preview-panel">
            ...
            <?php if ($startMode === 'existing'): ?><div class="alert"><h3>Next: opening conversion</h3><p>This saves the business shell first. Then map the existing chart and opening trial balance, reconcile unpaid invoices/bills and confirm cutover before posting.</p></div><?php elseif ($startMode === 'sample'): ?><p class="alert">This creates a separate, clearly marked sample company with fictional records. Existing companies are not changed.</p><?php else: ?><p>No prior balances or unpaid documents will be loaded. These books start empty.</p><?php endif; ?>
        </div>
    <?php endif; ?>

    <?php if ($activeStep >= 4 || $preview): ?>
        <section class="panel setup-chart-panel" aria-labelledby="starter-title">
            ...
            <?php if ($chartChoice === 'bring_own'): ?><p class="alert">Your existing chart is the opening-conversion source. The neutral chart below is the temporary setup shell; no existing account is reclassified or imported on this confirmation.</p><?php endif; ?>
            <div class="table-wrap" tabindex="0" role="region" aria-label="Starter chart of accounts"><table class="data-table"><caption><?= $chartChoice === 'bring_own' ? 'Temporary setup chart snapshot' : 'Accounts that will be created for this business' ?></caption><thead><tr><th scope="col">Code</th><th scope="col">Account</th><th scope="col">Type</th></tr></thead><tbody><?php foreach ($template['accounts'] as $definition): ?><tr><td><?= pl_e((string) $definition['code']) ?></td><td><?= pl_e((string) $definition['name']) ?></td><td><?= pl_e(ucfirst((string) $definition['type'])) ?></td></tr><?php endforeach; ?></tbody></table></div>
        </section>
    <?php endif; ?>

    <?php if ($preview): ?>
        <form action="<?= pl_e(pl_url('/onboarding')) ?>" method="post" class="actions setup-confirm-actions">
            <?= pl_csrf_field() ?><input type="hidden" name="action" value="confirm">
            <button class="button primary" type="submit"><?= $startMode === 'sample' ? 'Step 6: Create separate sample company' : ($startMode === 'existing' ? 'Step 6: Save and prepare opening conversion' : 'Step 6: Create business and accounts') ?></button>
            <a class="button secondary" href="<?= pl_e(pl_url('/onboarding?step=4')) ?>">Edit chart choice</a>
        </form>
    <?php endif; ?>
</section>
