...
<?php pl_ui_document_header('Goods receipt · '.$order['number'],'draft',static function () use ($order,$preview,$eligible): void { ?>
<a class="btn btn-ghost" href="<?= pl_e(pl_url('/purchasing',['id'=>$order['id']])) ?>">Cancel</a>
...
<?= pl_csrf_field() ?><?= pl_scope_fields($company) ?><?php pl_starter_hidden('id',$order['id']); pl_starter_hidden('request_key',$input['request_key']??bin2hex(random_bytes(20))); ?>
