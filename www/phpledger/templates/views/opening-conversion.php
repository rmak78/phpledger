...
?><details class="rounded-panel border border-border bg-surface p-4"<?= $review && (int) $review['review']['cutover_id'] === (int) $cutover['id'] ? '' : ' open' ?>><summary class="section-title">Cutover <?= pl_e($cutover['cutover_date']) ?> Â· Party mappings</summary>
...
<section class="rounded-panel border border-border bg-surface p-4"><h2 class="section-title mb-2">Converted opening debts</h2><?php if (!$items): ?><p>No opening debts have been converted. New businesses can start with their ordinary invoice and bill flows.</p><?php endif; ?>
...
<p class="text-xs text-ink-muted mt-3">Use the shared payment editor to review and allocate a payment across this partyâ€™s open documents. Opening conversion never adds a second opening journal.</p>
</section>
</section>
