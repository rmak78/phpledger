...
<section class="flex flex-col gap-4 py-5" aria-labelledby="modules-title">
    ...
    <p class="text-sm text-ink-muted">The accounting core, Accounts Receivable and Accounts Payable are bundled and always available. Purchasing and Inventory can be enabled below.</p>
    <section class="rounded-panel border border-border bg-surface p-4 text-sm"><h2 class="section-title">Keep your navigation simple</h2><p>Hide areas you do not use. Financial totals, existing records and authorised services are unaffected.</p>
    ...
    <section class="rounded-panel border border-border bg-surface p-4 text-sm" aria-labelledby="module-<?= pl_e($manifest['id']) ?>">
        <div class="flex items-start justify-between gap-3"><div><h2 class="section-title" id="module-<?= pl_e($manifest['id']) ?>"><?= pl_e($manifest['name']) ?></h2><p class="muted">Installed version <?= pl_e($manifest['version']) ?></p></div><?php pl_ui_badge($state['enabled']?'active':'inactive',$state['enabled']?($module['current']?'Enabled':'Upgrade review needed'):'Disabled'); ?></div>
        ...
        <form class="flex flex-wrap items-end gap-3 border-t border-border pt-3" method="post" action="<?= pl_e(pl_url('/modules')) ?>">
            ...
            <div class="field flex-1 min-w-48 max-w-sm"><label for="module-reason-<?= pl_e($manifest['id']) ?>">Reason for this change</label><input class="input" id="module-reason-<?= pl_e($manifest['id']) ?>" name="reason" maxlength="500" required value="<?= pl_e($moduleInput['reason']??'') ?>"></div>
    ...
    <div class="table-wrap" tabindex="0" role="region" aria-label="Module changes; scroll horizontally on small screens"><table class="table"><thead><tr><th>Module</th><th>Change</th><th>Reason</th><th>By</th><th>Recorded</th></tr></thead><tbody>
