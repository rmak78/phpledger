...
function pl_web_starter_purchasing(int $actorId, int $companyId, int $bookId, array $user, array $company, string $path, string $method): never
{
        ...
        try {
            ...
            if ($action === 'save') {
                if (pl_web_text($_POST,'editor_action')==='add_line' || isset($_POST['remove_line'])) {
                    ...
                    pl_form_failure(pl_workflow_url($path,['id'=>$id?:null,'new'=>$id?null:'1','return_filters'=>$filters]),pl_web_line_action($_POST),'',200);
                }
                $lines = [];
                ...
                unset($_SESSION['starter_purchase_bill_preview']);
                pl_notice('Supplier bill posted and receipt quantities matched.'); pl_redirect(pl_workflow_url('/ap', ['id' => $result['bill_document_id']]));
            ...
            pl_redirect(pl_workflow_url($path,$id?['id'=>$id,'return_filters'=>$filters]:$filters));
            ...
            pl_form_failure(pl_workflow_url($path,$query), $input, $error->getMessage());
        }
    }
    ...
    pl_render('purchasing', ['title' => 'Purchase orders', 'user' => $user, 'company' => $company, 'order' => $order,'selection'=>$selection,'filters'=>$filters,
        ...
        'form' => pl_form_state(pl_workflow_url($path,$id?['id'=>$id,'return_filters'=>$filters]:(isset($_GET['new'])?['new'=>'1','return_filters'=>$filters]:$filters))),
...
function pl_web_goods_receipt(int $actorId,int $companyId,int $bookId,array $user,array $company,string $method): never
{
    ...
    $id=pl_web_id($method==='POST'?$_POST:$_GET,'id'); $return=pl_workflow_url('/purchasing',['id'=>$id,'receive'=>'1','return_filters'=>$filters]);
    if ($method==='POST') {
        try {
            ...
            if (pl_web_text($_POST,'action')==='receipt_confirm') {
                ...
                pl_redirect(pl_workflow_url('/purchasing',['id'=>$id,'return_filters'=>$filters]));
            }
